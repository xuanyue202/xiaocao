...
remote writer在新报告或观点维护终态后，提前完成来源层语义分析与独立复核，使用`kol_trading_preparation.py`保存绑定来源hash、条件、期限、反证和覆盖缺口的`authority=0`准备包。它是新/变更正文的唯一完整阅读者。09:00与09:25分别复用此包，只处理各自 runtime 的冻结候选、账户和当前事实映射，不重新提炼或复读未变化正文；下游主审独立复核的是来源包绑定、当下映射和反证。缺包或`source_analysis_required`时回确定性基线并等待来源 owner，不由交易任务接管全文分析。准备包不是交易决策，不能预先认定开盘条件满足；正式决策仍遵守§2a完整来源、独立当下复核和发布要求。复用仍有效且适用的已发布判断。

此交接由逐对象`daily_trading_source_preparation_input_required`检查点触发，读回报告/manifest/准备包绑定并保存回执后才继续下一对象；不等待整批任务收尾。中断后只恢复持久化交接请求，不重放发布、提醒或Book效果。缓存缺失而已知内容未变时标为`source_revalidation_required`，复用原来源分析并补验缺失清单，不能把缓存失效冒充观点变化或当前判断批准。
...
来源语义准备、来源读回时效、当前正式决策是三个独立状态。相同hash的来源分析不会因为过了十分钟而失效；缓存补验不触发重新提炼。历史TTL仍24小时，09:00补验应覆盖11:30恢复窗口，保持真实as-of；刷新只按准确ID执行。`kol_trading_preparation.py status --ready-through ... --opening-draft ...`分别报告复用、历史缺口、条件草稿和当前policy状态，不能把source prepared称为交易ready。

09:20前完成条件草稿的理由、基础调整、失效条件、开盘情景及独立来源复核；仅有null占位符的模板不算预计算完成。若当时已有足够事实可形成适用的正式判断，提前发布且按真实条件选择覆盖本交易时段的valid_until；不能为了续命改时间，也不强制产出中性。确需开盘事实的部分明确留待最终适配；09:25只核对变更与当前条件，复用已审核来源，不重新输出全篇语义。新/变更来源仍完整读取并独立复核。

完整request、候选、回执保存文件，stdout默认只给紧凑通知、路径、hash及实际deadline；brief保留全部候选的决策相关字段，其他细节按需从原件读取。09:20前保存恢复检查点（原进程ID、来源/草稿路径、仍缺事实、下一动作），09:24起仅增量读取和紧凑状态。自动压缩时点由客户端决定，任务不承诺禁止压缩；必须通过减少输入和提前checkpoint避免在开盘依赖满上下文。FILLED/CANCELLED与策略SKIPPED混合也是终态，UNKNOWN仍单独未决。延迟和支持层降级独立报告。
...
- **阶段一执行缝**：`src/xiaocao/live/trading_execution.py`、
  ...
  可审计的 broker-neutral probe/prepare/reconcile/recover 边界。09:00 live
  ...
  BUY 最早 09:25 可提交到 APP 柜台排队；未报/待报/正报的唯一委托号只证明柜台接受，不证明交易所已接收或成交。SELL仍从09:30起。全新BUY提交前通过账户绑定、下述针对本批的原生读回与
...
- **实盘全生命周期**：`BookBOwnershipEvidence` 只接收 native broker 已证明的
  ...
  facts 的历史/恢复分支仍可用严格三表冻结证明；全新BUY使用上文独立buying_power回执，只消费APP直接显示的可用资金。两者都不改变Book-B NAV、敞口或批次额度；不取消、重试或收编额外委托以消除差额。完整对账回执持久化实际闭合等式的
...
- 09:00 runner 先做无敏感信息 preflight，并在每个 submit-capable 资金门重新现读两项 Keychain runtime material；只把兼容映射作为函数参数传给 `safety.py`，不得写入 `os.environ`、argv、日志、回执、takeover capsule 或任何落盘证据。Keychain 可读本身不是订单授权；仍必须有同一密钥验证通过且未过期、未越权的授权文件。
- 两个 Keychain 项与 HMAC 授权都属于同一 macOS 登录主体，因此这是用户明确批准的**同主体运行门**，不是两个安全主体的密码学隔离；不得再声称“agent 从技术上绝对无法自签”。交互命令 `scripts/configure_live_capital_keychain.py` 用于创建/启用固定 runtime 项，`scripts/authorize_live.py` 用于铸造 scoped authorization；09:00 Automation 的运行契约禁止调用两者、创建/旋转密钥或铸权。
...
- **现状**：阶段一执行缝已在任何 broker adapter 之前调用
  `require_capital_action(...)`；独立 09:00 Automation 只启动隔离 live seam，
