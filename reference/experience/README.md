...
这些是**叙述/判断先验**，进 **agent 皮层**与**能力飞轮（当假设）**，原始断言权威始终为 0，**永不**自动改参。永久策略/参数升级仍须研究 PASS 与人工门。2026-09-06 用户另行确认了**有界当下判断接口**：已发布来源经当下检查、Astra 分析及主 Agent 独立复核，可在既有合格 Book-B 模拟/实盘决策中表达受限买入/退出判断，不要求每条判断先过消融；不是原始先验直接控制交易。范围、时效、账户风险与不可绕过的执行门统一见 `docs/OPERATING_CONTRACT.md` §2a，操作见 `.codex/skills/xiaocao-trading/references/kol-trading-judgment.md`。
