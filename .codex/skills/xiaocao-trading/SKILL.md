...
`TREND_REBALANCE_R` is a paired morning switch cue, not an EOD unpaired sell.
An already-held `external` T row is not sold unpaired at EOD. After T+1, the
next morning `paper_record.py --trend-only` may switch an `external` or
rebalance-due row only when a replacement candidate is fillable in the same run:
record the old row as `TREND_POSTURE_MISMATCH` or `TREND_REBALANCE_R` /
`paired_morning_switch`, then buy the new aligned/neutral representative. If no
replacement is fillable, keep the old row open to preserve trend exposure.
Ordinary ranking drift is not a switch signal, because Book T is a low-turnover
exposure sleeve and fees matter. T must not consume Book-B
...
1. `scripts/live_recommend.py` waits until the 9:25 auction snapshot is usable, writes the recommendation, captures ★/★B forward signals, and records bounded news/evidence snapshots for the full candidate set plus current open Book-B positions (`INTELLIGENCE_EVIDENCE_TIMEOUT_SEC=60`, normally expected to finish well under that; same-day cache hits are reused only while fresh, default TTL 30 minutes).
2. `kronos_screen/scripts/paper_record.py` confirms the Book-B paper buy after the opening execution window and runs with `--intelligence-trade on`. The intelligence leg consumes only structured `agent_review` evidence from `signal_snapshots.jsonl`: it can rerank all latest candidates by `rank_score + ai_intelligence_short_score × ai_score_bonus`, can let a high short-line score replace a lower base ★B pick, and can skip a candidate only for a model-judged taxonomy hard-veto (`AI_HARD_VETO`). If no structured review exists it must degrade to the original ★B picks exactly; keyword/news sentiment must not participate in the trade decision. Treat `basket` as the **abandon bound only, never the fill assumption**. The fill model is a realistic paper limit order: `L = min(open × (1 + 0.5%), basket_price)`; after the 9:30-9:31 window settles, if the window trades through `L`, fill at `min(window VWAP, L)`. If the initial low limit is not filled or would be rejected as too far from the tape, check the latest opening-window price as the real-time retry proxy; when that price is still `<= basket_price`, fill at that real-time price and audit `retry_realtime_after_limit_reject`. If the retry price is above `basket` or unavailable, the pick is **skipped** and audited in `output/live/paper_skips.jsonl` (`SKIPPED / LIMIT_NOT_REACHED`). If minute data is unavailable it falls back to `L` and marks `fill_fallback`. It also records **book A** — the validated reference policy (same final entry fill as Book B, sell at next close, no stop) — as a parallel virtual book, and applies a **kill-switch**: if book A's last 5 exit-days cum return < -3% it halves book B's deploy, < -5% it pauses Book-B buys entirely (book A and data capture always continue).
3. The morning orchestration then runs `paper_record.py --trend-only` for Book T. This records new trend slots into `paper_account_T.json` and, when an already-held `external` T row is T+1 eligible or a row is rebalance-due and a replacement candidate is fillable, performs the paired trend switch in the same run. Switch candidates consume fillable replacements before ordinary empty-slot fills. No Book-T candidate, an already-full T book with no paired switch, or a skipped T fill is normal paper state and not a Book-B anomaly.
...
| `basis` | Short recommendation basis: mode/score, flags, any produced small-grass indicators, and when present `ai_intelligence_trade_reason` / `ai_intelligence_trade_score` |
...
candidate skipped by `AI_HARD_VETO`, insufficient cash, or suspicious data/output behavior.
...
Reply in Chinese with a concise cash/equity/open-position summary. Treat `T+1_blocked` as diagnostic only. Sell execution is **staged**: intraday checkpoints only execute `HARD_STOP` (drawdown ≥ 8% hard floor), structured intelligence hard-veto exits (`AI_EVENT_RISK_EXIT`), or liquidity escapes; ordinary trailing-stop / composite deterioration is only **diagnosed** intraday (`defer:<reason>` in the status column, `SELL_DEFERRED` in alerts) and executed at the 14:55 discipline pass — a deferred diagnosis is normal, not an anomaly. `AI_EVENT_RISK_EXIT` must come from structured `agent_review.veto_flags` taxonomy evidence with sufficient severity/confidence/recency; it is not keyword matching, and it still respects T+1 and liquidity constraints. If a fresh event risk is T+1 blocked, report the unresolved event risk rather than treating it as an executed sell. Highlight only actual executed sells, `HARD_STOP` triggers, `AI_EVENT_RISK_EXIT` triggers/blocks, script failures, missing/corrupt account or position files, or other real anomalies. Book A positions (`book="A"`) are settled separately by `settle_book_a.py` and never appear in the monitor.
