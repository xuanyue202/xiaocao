...
def main() -> int:
    ...
    parser.add_argument("--reuse-open-window", action="store_true", help="Resolve a supported branded entry for an already-open course without issuing another ticket")
    parser.add_argument("--subscription-id", help="Emit an exact runner response without retyping IDs")
    args = parser.parse_args()
    try:
        result = resolve_launch_plan(args.source_url, expected_identity=args.expected_identity, reuse_open_window=args.reuse_open_window)
    ...
    if args.subscription_id:
        ...
        if result.get("mini_program_name"):
            result["browser_response"]["mini_program_name"] = result["mini_program_name"]
