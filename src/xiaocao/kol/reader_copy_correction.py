...
吕晓彤并非笼统看多科技。他明确主张降低存储芯片、通信和光模块仓位，停止使用倍数产品，同时保留人工智能、机器人以及受长鑫扩产驱动的半导体设备方向。

他的核心判断是：存储芯片涨价可能受到苹果等强势下游议价、韩国杠杆资金出清和新增供给压制；半导体设备赚的是长鑫扩产采购，不应与存储芯片涨价逻辑一起处理；长期人工智能方向可以保留，但应使用非杠杆股票或ETF表达。
...
吕晓彤把这次全球科技急跌解释为拥挤交易、融资盘和量化资金相互踩踏形成的快速去杠杆，而不是单一公司的普通回调。他明确反对长期持有两倍、三倍科技多头产品：长期看多科技并不能抵消高波动路径中的损耗和强平风险。

他认为非杠杆行业ETF与个股必须分开处理。ETF具有成分调整机制，但这不等于任何买入价都一定快速修复，因此既不要恐慌卖出，也不要仅凭“必然涨回”追加仓位。他还把苹果（AAPL）列为下跌后的候选机会，但没有给出价格、估值或确认条件，目前只能观察，不能直接执行。
