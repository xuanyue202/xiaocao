...
def test_download_confirmation_recovers_detached_route_readback_within_bound(
    tmp_path,
):
    entry = _pdf_entry()
    entry["provider_file_id"] = "123456789012345"
    ...
    def browser_runner(command, **_kwargs):
        nonlocal confirmation_evals, route_readback_evals
        nonlocal foreground_open_calls, bind_calls, trigger_calls
        tail = command[3:]
        if tail[:1] == ["open"]:
            if tail[-2:] == ["--window", "foreground"]:
                foreground_open_calls += 1
    ...
    assert result["status"] == "completed"
    assert confirmation_evals == 2
    assert route_readback_evals == 3
    assert foreground_open_calls == 3
    assert bind_calls == 3
    assert trigger_calls == 1
