...
def test_repair_after_open_resumes_same_plan_without_new_review(tmp_path):
    config, plan, _ = _seed(tmp_path)
    calls = []
    result = run_book_b_live_recovery(config, plan_id=plan.plan_id, now=lambda: OPEN + timedelta(minutes=1),
        prepare_only=_prepared, execute=lambda p: calls.append(p.plan_hash) or ExecutionReceipt(
            p.plan_id, p.plan_hash, ExecutionState.FILLED, filled_shares=p.shares),
        review_rendezvous=lambda _: pytest.fail("same-plan recovery needs no new semantic review"))
    assert result.status == "completed" and calls == [plan.plan_hash]
...
def test_checkpoint_expires_unclaimed_opening_plan_without_broker_access(tmp_path):
    ...
    result = reconcile_open_book_b_plans(config.state_dir, trade_date=config.trade_date,
        now=plan.recovery_deadline, execute=lambda _: pytest.fail("local unclaimed closure"))
...
def test_withdrawn_opening_metadata_does_not_expire_plan_at_checkpoint(tmp_path):
    config, plan, _ = _seed(tmp_path)
    path = next((config.state_dir / "plan_intents").glob("*.json"))
    payload = json.loads(path.read_text())
    payload["opening_preparation_deadline"] = OPEN.isoformat()
    path.write_text(json.dumps(payload))
    result = reconcile_open_book_b_plans(config.state_dir, trade_date=config.trade_date,
        now=OPEN + timedelta(minutes=5), execute=lambda _: pytest.fail("unclaimed plan"))
    assert result == () and plan.plan_id in open_execution_plan_ids(config.state_dir)
